...
search.OMIM <- function(gene){
  ...
  if (length(jsonlite::fromJSON(jsonlite::toJSON(content(r)))$omim$searchResponse$geneMapList$geneMap$phenotypeMapList)!=0){
    ...
    if (length(phen)==0) return(NA)
    else return(unlist(phen))
  }
  else return(NA)
}

#' @title Look up genes in OMIM
#' @descriptionCheck if any gene in your list has an entry in OMIM showing relevant phenotypes for a trait of interest
#'
